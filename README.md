...
> **Status: Phase 1 + 2 + 3 implemented.** The loop — *file a Demand → run an agent in an isolated git worktree → stream live logs → capture the diff → human review → commit → push → PR* — runs end to end, with **concurrency-safe project locks + crash recovery** (safe to run multiple workers). Code lives in [`agentplane/`](agentplane/) (pnpm monorepo); the full specification is in [`agentplane/docs/`](agentplane/docs/). Remaining phases (locks runtime, CI/CD, multi-user RBAC, daily planning) follow the roadmap in [`docs/architecture/11-mvp-roadmap.md`](agentplane/docs/architecture/11-mvp-roadmap.md).
>
